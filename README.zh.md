...
MCP 服务器通常需要密钥（API key、token）。ai-switch 只迁移**接线** —— 服务器名称、command、args 以及 env 变量*名称* —— 绝不在工具之间或报告中复制密钥**值**。迁移后，`ai-switch-report.md` 会列出已迁移服务器所需的全部凭证，你只需为新工具设置相同的环境变量即可。在迁移后的配置中，任何明文值都会被**改写为 `$NAME` 引用**（值本身绝不复制），并在报告中列出，便于你在环境中设置它，若是密钥则进行轮换。
