...
MCP サーバーは通常シークレット（API キー、トークン）を必要とします。ai-switch は**配線** —— サーバー名・command・args・env 変数の*名前* —— のみを移行し、シークレットの**値**はツール間にもレポートにも決してコピーしません。移行後、`ai-switch-report.md` が移行済みサーバーに必要な認証情報の一覧を示すので、同じ環境変数を新しいツールに設定するだけで済みます。設定に直書きされた実際のシークレットは（マスクして）フラグ付けされ、環境変数へ移してローテーションするよう促します。
